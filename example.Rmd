...
Citation info here: [![DOI](https://zenodo.org/badge/659269746.svg)](https://zenodo.org/badge/latestdoi/659269746){target="_blank"}
...
The map below shows the locations of underserved communities, areas within 1 mile of those communities, and flowlines (for identifying downstream communities).
...
mapview(tb_equity, col.regions = "black", layer.name = "Underserved Communities") + mapview(tb_flowlines, col.regions = "blue", layer.name = "Flowlines") + mapview(buffer_under, col.regions = "red", layer.name = "1 mile buffer")

# Entire bay
mapview(bay, col.regions = "red", layer.name = "Tampa Bay Shoreline (1 mile)") + mapview(tb_equity, col.regions = "black", layer.name = "Underserved Communities")

# Bay segments
mapview(bayseg, zcol = "BAY_SEG", col.regions = list("red","orange","yellow","green","blue","violet","brown"), layer.name = "Bay Segment Shoreline (1 mile)", label = "BAY_SEG") + 
  mapview(tb_equity, col.regions = "black", layer.name = "Underserved Communities")

# Counties
mapview(county, zcol = "NAME", col.regions = list("red","orange","yellow","green","blue","violet"), layer.name = "County (Watershed Boundary)", label = "NAME") + 
  mapview(tb_equity, col.regions = "black", layer.name = "Underserved Communities")

# Cities
mapview(city, zcol = "minorciv_1", layer.name = "Municipalities (Watershed Boundary)", label = "minorciv_1") + 
  mapview(tb_equity, col.regions = "black", layer.name = "Underserved Communities")

# Watershed
mapview(watershed, layer.name = "Tampa Bay Watershed") + 
  mapview(tb_equity, col.regions = "black", layer.name = "Underserved Communities")
